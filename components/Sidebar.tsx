...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex w-56 shrink-0 bg-white border-r border-slate-200 flex-col h-screen sticky top-0">
        {/* Logo */}
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-white border-b border-slate-200 flex items-center justify-between px-4 h-14">
        <Link href="/calendar" className="flex items-center gap-2 no-underline">
          <span className="w-7 h-7 bg-blue-600 text-white rounded-lg flex items-center justify-center text-sm font-black">J</span>
          <span className="text-base font-extrabold tracking-tight text-slate-900">Jobro</span>
        </Link>
        <form method="POST" action="/api/auth/signout">
        ...
        {BOTTOM_NAV.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                'flex-1 flex flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-medium no-underline transition-colors',
