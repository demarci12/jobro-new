  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-extrabold tracking-tight text-slate-900">Quotes</h1>
      </div>
                ...
                return (
                  <tr key={q.id} onClick={() => router.push(`/bookings/${q.booking_id}`)} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 cursor-pointer transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${STATUS_CLS[q.status] ?? STATUS_CLS.draft}`}>
                        {q.status}
                      </span>
                    </td>
                  </tr>
